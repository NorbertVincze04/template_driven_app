...
tenantRouter.patch("/pricing", tenantMiddleware, authMiddleware, async (req, res) => {
  ...
  if (normalizedPlans.some((plan: { name: string | null; price: string | null }) => !plan.name || !plan.price)) {
    return res.status(400).json({
      success: false,
  ...
  const pricing = {
    sectionLabel: normalizeText(req.body?.sectionLabel, 80) || "Services & pricing",
    title: normalizeText(req.body?.title, 160) || "Care that meets you where you are.",
    description: normalizeText(req.body?.description, 400) || "Choose the appointment that fits your goals.",
